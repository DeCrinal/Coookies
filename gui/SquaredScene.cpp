...
void SqScene::get_level_data(int choosed_lvl)
{
    ...
    QBrush board_brush(QColor(98,87,114));
    ...
    case(3):
    case(4):
    case(5):input.open("fifth.txt");break;
    case(6):input.open("six.txt");break;
    case(7):
    case(8):
    case(9):
    case(10):
...
void SqScene::createBoards(){
    ...
    if(in.is_open()){
        qDebug()<<"Variables opened";
    }else{
        qDebug()<<"Variables wasn't open";
    }
    in >> choosedLvl;
    in.close();
    if(choosedLvl==1){
...
void SqScene::setVector(std::string vector_from){
    if(vector_from!="root")
